...
class ObjectTracker:
    def __init__(
        ...
        nms=None,
        tsize=None,
        track_thresh=0.50801,
        track_buffer=60,
        match_thresh=0.82701,
        aspect_ratio_thresh=1.27225,
        min_box_area=3,
        mot20=True,
        fps=30,
    ):
    ...
    def process_image(self, img, frame_id=0):
        ...
        if outputs[0] is not None:
            ...
            # 1단계: 이 프레임의 모든 사람 크롭을 먼저 모으기 (아직 추론 안 함)
            valid_targets = []  # (tid, bbox, score, image_resized) 튜플 리스트
            for t in online_targets:
                ...
                if tlwh[2] * tlwh[3] > self.tracker.min_box_area:
                    ...
                    valid_targets.append((tid, bbox, t.score, image_resized))

            # 2단계: 모은 크롭들을 한 번에 배치로 추론 (프레임당 GPU 호출 1번)
            if valid_targets:
                person_crops = [v[3] for v in valid_targets]
                age_gender_embeds = self.predictor.age_gender_recognizer.run(person_crops)

                # 3단계: 배치 추론 결과를 각 사람에게 다시 나눠 적용
                for (tid, bbox, score, _), (age, gender, embed_vec) in zip(valid_targets, age_gender_embeds):
