...
class VideoRecognizer:
    ...
    def run_batch(self, person_crops: list) -> list:
        """
        YOLOX에서 크롭된 person 이미지 N장을 배치로 한 번에 추론.
        YOLOv8 detection 스킵, GPU 왕복 1번.

        Args:
            person_crops: List[np.ndarray] — BGR 크롭 이미지 리스트 (resize 불필요)

        Returns:
            List of (age: float, gender: str, embed_vec: np.ndarray)
        """
        if not person_crops:
            return []
        ...
        # face 채널 자리는 None 리스트 → prepare_classification_images 내부에서 zeros 처리
        none_list = [None] * len(person_crops)

        person_input = prepare_classification_images(
            person_crops,       # BGR np.ndarray 그대로 OK (내부에서 BGR→RGB, letterbox 처리)
            mivolo.input_size,
